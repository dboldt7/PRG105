...
state_capitals.write("Salem\n")
state_capitals.write("Boise\n")
state_capitals.write("Austin\n")
...
for count in range (1, books + 1):
    title = input("say a title ")
    author = input('say an author ')
...
try:
    hours = input("say a number")
    number = 2 * int(hours)
except ValueError:
    print("That number has to be an integer")
...
try:
    super_hero = open('superheroes.txt', 'r')
except IOError:
    print("The file shouldn't exist")
